...
public class UnionPolygon {
	...
	public static void main(String[] args) throws IOException {
		...
		SparkConf conf = new SparkConf().setAppName("Group22-Union Polygon");
	...
	public static void unionPolygons(JavaSparkContext sc, String input_file, String output_file) throws IOException {
		...
		JavaRDD<Geometry> poly_rdd = input_data.mapPartitions(LocalUnion); // local
																			// union
																			// of
																			// rectangles
		JavaRDD<Geometry> poly_rdd_rep = poly_rdd.repartition(1);
		JavaRDD<String> final_polygons = poly_rdd_rep.mapPartitions(GlobalUnion).repartition(1).distinct(); // global
																											// union
																											// of
																											// polygons

		// To sort the resulting coordinates
		final_polygons.map(PointDouble.ToPointDouble).mapPartitions(PointDouble.SortRDD).map(PointDouble.PointToString)
				.saveAsTextFile(output_file);
	}

	// Iterate over polygons and create a collection. Perform union of the
	// collection.
	...
	// Converts the input data (diagonal coordinates) to rectangles and perform
	// the union of the collection of rectangles
	public static FlatMapFunction<Iterator<String>, Geometry> LocalUnion = new FlatMapFunction<Iterator<String>, Geometry>() {
		...
		public Iterable<Geometry> call(Iterator<String> input_data) {
			...
			while (input_data.hasNext()) {
				 ...
				 * (x1,y1)-------------------(x2, y1)
				 * 
				 * x1,y1 is not necessarily the left bottom corner. It can be
				 * any of the corner
				 */
				Geometry polygon = geom.createPolygon(new Coordinate[] { new Coordinate(x1, y1), new Coordinate(x1, y2),
